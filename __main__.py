...
from Nexus_config.nexus import NexusConfig
...
def createApp (connect):
    appname = "nexususerbot" + str(time() * 1000)[-4:].replace(".", "") + str(random.randint(0,500))
    try:
...
Photo = await Client.upload_file(file='hqdefault.jpg')
...
# Reponu şifrele #
    nexusrepo = 'aHR0cHM6Ly9naXRodWIuY29tL0N5YmVyVXNlckJvdC9DeWJlclVzZXJCb3Q='
    nexusrepo2 = nexusrepo.encode('ascii')
    nexusbytes = base64.b64decode(nexusepo2)
    nexusrc = cyberbytes.decode('ascii')

if os.path.isdir(nexusConfig.DESTINATION):
